...
**共享范围:只有 owner + token。** 各 app 自己的 repo / 分支 / 数据文件路径都保留不动。
(令牌会定期过期、是真正的重复劳动;repo 设一次就固定。)
...
| 影集 | Album-Journal | https://nickkklian.github.io/Album-Journal/ |
...
1. 在 **NickkkLian 账号**下新建一个仓库(建议名 `hub`,网址就会是 `https://nickkklian.github.io/hub/`)。
2. 把本项目代码推上去。
...
10 个 app 各加了一行附加引导(注释标记 `[hub-portal]`),逻辑:**自己的令牌为空时**,从
`pha-config` 读 `owner+token` 填入;已自填的不覆盖;不碰 repo/path。补丁清单(每个文件 1 处):
...
> 本地补丁已改在 `~/Desktop/hub-apps/<App>/index.html`,并在同 origin 环境实测通过
> (令牌自动拾取、repo/path 不变、已填不覆盖、原功能不受影响)。**尚未推送各 app 仓库**,
> 由站长确认后推。
