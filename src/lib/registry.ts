...
/**
 * 卡片分组（站长 2026-08-07 定的五组）。
 * 分组是**展示层的归类**，跟 app 本身的权限/隐私无关——别拿它当判据，
 * 「永不公开」的名单权威仍在 personal-hub/HUB-APPS.md §2。
 */
export type GroupId = "admin" | "pursuit" | "io" | "record" | "tool";

/** 分组的显示顺序与名字。改这里就能调整首页从上到下的次序。 */
export const GROUPS = [
  { id: "admin", label: "管理层", labelEn: "Admin" },
  { id: "pursuit", label: "追光类", labelEn: "Chasing Light" },
  { id: "io", label: "输入输出类", labelEn: "Input & Output" },
  { id: "record", label: "记录类", labelEn: "Records" },
  { id: "tool", label: "工具类", labelEn: "Tools" },
] as const satisfies readonly { id: GroupId; label: string; labelEn: string }[];

/* 编译期兜底：首页是照着 GROUPS 一组组渲染的，**GROUPS 里没列到的组，它的卡片会静默消失**
   （不报错、不留白，就是没了——最难发现的那种）。所以要求 GROUPS 覆盖 GroupId 的每一个取值。
   验证过它会失败：注释掉 GROUPS 里任意一行，tsc 立刻报
   `Type "tool" does not satisfy the constraint 'never'`。
   注意别把 GROUPS 标成 `{id: GroupId}[]`——那样下面这行永远是 never，检查就成了摆设。 */
type AssertNever<T extends never> = T;
// 必须 export：本项目开了 noUnusedLocals，不导出的话它在**正常状态下**也报错、把构建卡死。
export type _EveryGroupIsListed = AssertNever<Exclude<GroupId, (typeof GROUPS)[number]["id"]>>;

export interface AppLink {
  /** 内部标识 */
  id: string;
  /** 属于哪一组。**必填**——漏了或拼错会被 `tsc --noEmit` 挡下，不会静默从首页消失 */
  group: GroupId;
